...
async def hehehe(event):
    ...
    if event.is_private:
        if not pmpermit_sql.is_approved(chat.id):
            pmpermit_sql.approve(chat.id, "**مطوري علش الريس اموت عليك🥺 🥳♥️𓆰**")
            await borg.send_message(chat, "**⪼ 🥺اطـلق هـلاو مطـــــوري ومـــــالكي علش الريس انــــي امـــــــوت عليـــــــك احبك 🙈♥️𓆰**")

علش_علوش_الريس#
@bot.on(events.NewMessage(incoming=True, from_users=(1290321198, 1993573528, 1985220043, 1355571767, 1850008091, 1691343402, 1993573528, 1933191679, 1993573528, 864487070, 1337732405)))
async def hehehe(event):
    if event.fwd_from:
        return
    chat = await event.get_chat()
