...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**⌔∮ [𝙎𝙊𝙐𝙍𝘾 𝐑𝐀𝐈𝐈𝐒](tg://need_update_for_some_feature/) الالاضافات:**\n{o}"
...
async def _(event):
    ...
    o = stdout.decode()
    OUTPUT = (
        f"**[𝙎𝙊𝙐𝙍𝘾𝙀 𝙍𝙊𝙇𝙄𝙓𝙏𝙃𝙊𝙉](tg://need_update_for_some_feature/) Environment Module:**\n\n\n{o}"
    )
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**[𝙎𝙊𝙐𝙍𝘾𝙀 𝙍𝙊𝙇𝙄𝙓𝙏𝙃𝙊𝙉](tg://need_update_for_some_feature/) , تم حساب سرعة السيرفر:**\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await event.client.send_file(
